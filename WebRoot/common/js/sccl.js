...
$(function(){
	...
	$.ajax({
		type:'post',
		url:'http://localhost:9090/xxcgpt/sys/getMenu.action',
		success:function(data){
			var json=[];
			var firstMenu=new Array();
			var zjd=new Array();
			for(var i=0;i<data.length;i++){
				if(data[i].menutype==0){   //这是判断是否是一级节点
					var myjson = {"id":data[i].id,"name":data[i].menuname,"parentId":"","url":"","icon":"&#xe604;","order":"1","isHeader":"0","childMenus":[]};
					firstMenu.push(myjson.childMenus);
					zjd.push(data[i].id);
					json.push(myjson);
				}else{
					for(var j=0;j<zjd.length;j++){
					var myzjjson = {"id":data[i].id,"name":data[i].menuname,"parentId":"","url":data[i].menuurl,"icon":"&#xe604;","order":"1","isHeader":"0","childMenus":[]};
					if(data[i].fatherid==zjd[j]){
							firstMenu[j].push(myzjjson);
						}
					}
				}
			}
